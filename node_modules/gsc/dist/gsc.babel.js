...
var gsc = function () {
   ...
   * @private
   */
  var _dcUrl = 'http://hub.geosmartcity.eu/' + 'gsc-datacatalogue/datacatalogservlet';
   ...
   */
  mod.doPost = function (actionName, requestData) {
...
gsc.dataset = function () {
   ...
   */
  mod.list = function (dataSourceId, dataSetId, dataSetName) {
   ...
   */
  mod.create = function (dataSetName, realName, dataSourceId, description, toBeIngested, refreshInterval, url) {
   ...
   */
  mod.update = function (dataSetId, dataSetName, realName, dataSourceId, description, toBeIngested, refreshInterval, url) {
...
gsc.datasource = function () {
   ...
   * @public
   */
  mod.create = function (datasourcename, organization, type, description, updated, url, username, password, ipaddress, schema, port, path) {
   ...
   */
  mod.list = function (datasourceId, organization, datasourceName, includeDetail) {
    ...
    if (params.iddatasource === undefined && params.organization === undefined) {
      return gsc.util.errorPromise('Parameter datasourceId or organization must be present in request');
    }
   ...
   */
  mod.update = function (datasourceId, datasourcename, organization, type, description, updated, url, username, password, ipaddress, schema, port, path) {
...
gsc.download = function () {
  ...
  mod.create = function (map, layer) {
    ...
    mod.dragBox.on('boxend', function () {
      ...
      mod.layer.getSource().forEachFeatureIntersectingExtent(extent, function (feature) {
...
gsc.editFeatures = function () {
  ...
  mod.create = function (divContainer, divContent, aCloser, classButton, mapObject, wfs) {
      ...
      case 'btnEditAttributes':
        ...
        mod.interactionSingleSelect.on('select', function (evt) {
          if (evt.selected.length == 1) {
            ...
            mod.popupContent = '';
            attTitle = [];
            mod.popupContent = '<table id="feature" ' + 'class="table table-bordered table-hover" ' + 'style="clear: both"><tbody>';
            Object.getOwnPropertyNames(mod.selectedFeature.getProperties()).forEach(function (val, idx, array) {
              if (val != 'bbox' && val != 'geometry') {
                var i = 0;
                attTitle.push(val);
                mod.popupContent += '<tr>' + '<td width="35%">' + val + '</td>' + '<td width="65%">' + '<a href="#" id="' + val + '" class="edit' + i + '" ' + 'data-type="text">' + mod.selectedFeature.getProperties()[val] + '</a>' + '</td>' + '</tr>';
                i++;
              }
            });
            ...
            attTitle.forEach(function (val, idx, array) {
              ...
              mod.popupContent += 'gsc.editFeatures.selectedFeature.set("' + attTitle[idx] + '", newValue, false);';
              mod.popupContent += 'gsc.editFeatures.transactWFS("update", ' + 'gsc.editFeatures.selectedFeature);';
              mod.popupContent += '}';
              mod.popupContent += '})';
            });
            mod.popupContent += '});</script>';
            mod.content.html(mod.popupContent);
            var popupCoordinates = mod.selectedFeature.getGeometry().getClosestPoint(evt.mapBrowserEvent.coordinate);
...
gsc.layer = function () {
   ...
   * @returns {Promise.<Object>}
   */
  mod.create = function (layerName, dataSetId, description, metadataFile, sld) {
   ...
   * @returns {Promise.<Object>}
   */
  mod.update = function (layerId, layerName, dataSetId, description, metadataFile, sld) {
   ...
   */
  mod.list = function (dataSetId, layerId, layerName) {
...
gsc.map = function () {
  ...
  mod.infoOnFeatureEvent = function (evt) {
    document.getElementById(this.nodelist).innerHTML = 'Loading... please ' + 'wait...';
    var view = mod.olMap.getView();
    var viewResolution = view.getResolution();
    var source = this.layer.getSource();
    var url = source.getGetFeatureInfoUrl(evt.coordinate, viewResolution, view.getProjection(), { 'INFO_FORMAT': 'text/html',
      'FEATURE_COUNT': this.maxFeaturesNumber
    });
    if (url) {
      document.getElementById(this.nodelist).innerHTML = '<iframe seamless ' + 'src="' + url + '"></iframe>';
    }
  };
...
gsc.user = function () {
   ...
   */
  mod.register = function (email, username, password, confirmpassword, organizations) {
   ...
   * @return {Promise} [description]
   */
  mod.changePassword = function (username, oldpassword, newpassword, confirmnewpassword) {
...
gsc.util.isArrayWithContent = function (arrayCandidate) {
  if (arrayCandidate !== undefined && jQuery.isArray(arrayCandidate) && arrayCandidate.length > 0) {
    return true;
  } else {
    return false;
  }
...
//# sourceMappingURL=gsc.babel.js.map
